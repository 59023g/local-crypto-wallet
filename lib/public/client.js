...
const writeToLocalStorage = async () => {
  try {
    ...
    document.querySelector( '#writeToLocalStorage-notification' ).textContent = error.message
  }
...
const validateParseDate = async ( input_date_then ) => {
  ...
  if ( !input_date_then ) { throw { target: 'input_date_then', message: 'Date: Cannot be empty' } }
  if ( isNaN( rawDate ) ) {
    throw {
      target: 'input_date_then',
      message: 'Date: Not a valid Date. See ISO 8601 standard. Or try 2018/01/01 17:50:33 '
    }
  }
...
const exportJsonUrl = async () => {
  ...
  textArea.className = 'show table-cell'
...
const addInputEventListeners = async () => {
  let inputs = document.querySelectorAll( 'input' )
  addEventListenerList( inputs, 'blur', await validateFormOnClient )

}

const removeAllNodes = async ( selector ) => {
  let selectorNodes = document.querySelectorAll( `${ selector }` )
  for ( var i = 0; i < selectorNodes.length; i++ ) {
    selectorNodes[ i ].remove()
  }
}

// TODO - refactor interesting problem here
let success = {}
const validateFormOnClient = async ( e ) => {
  try {
    switch ( e.target.id ) {
      case 'input_date_then':
        await validateParseDate( e.target.value )
        if ( document.querySelector( '.error-input_date_then' ) ) removeAllNodes( '.error-input_date_then' )
        success.input_date_then = 'input_date_then'
        break;
      case 'input_symbol_pair':
        await validateParseSymbol( e.target.value )
        if ( document.querySelector( '.error-input_symbol_pair' ) ) removeAllNodes( '.error-input_symbol_pair' )
        success.input_symbol_pair = 'input_symbol_pair'
        break;
      case 'input_quantity':
        success.input_quantity = 'input_quantity'
        break;
    }

    if ( Object.keys( success ).length >= 3 ) {
      document.querySelector( '#writeToLocalStorage' ).className = 'table-cell submit bg-white color-black'
    } else {
      document.querySelector( '#writeToLocalStorage' ).className = 'table-cell submit'
    }
  } catch ( error ) {
    document.querySelector( '#writeToLocalStorage' ).className = 'table-cell submit'
    delete success[ error.target ]
    document.querySelector( '#writeToLocalStorage-notification' ).innerHTML += `<div class="error-${error.target}"> ${ error.message }</div>`
  }
}
      ...
      for ( let l in binanceTickerData ) {
        ...
        if ( binanceTickerData[ l ].symbol === entry.input_symbol_pair ) {
          ...
          if ( now_price_crypto > existing_now_price ) {
            ...
            dom_meta_price_crypto_now.className = 'table-cell mobile-cell highlight-cell dom_meta_price_crypto_now red'
          } else {
...
const validateParseSymbol = async ( input_symbol_pair ) => {
  ...
  if ( formattedSymbol.length < 6 || formattedSymbol.length > 8 ) throw { target: 'input_symbol_pair', message: 'Symbol: Must be 6 to 8 characters long. For example: ZRXBTC or zrx/btc or ZRX/BTC' }
  if ( window.binance_symbols.indexOf( formattedSymbol ) === -1 ) throw { target: 'input_symbol_pair', message: `Symbol: ${ formattedSymbol } Not found on Binance. Or it may not be in correct format. Try ZRXBTC or zrx/btc or ZRX/BTC. If not, contact hi@mep.im if interested in more exchanges.` }
...
const appendEntryToTable = async ( entry ) => {
  ...
  entriesDiv.insertAdjacentHTML( 'beforeend', `
    <div id='entry-${ entry.id }' class='table table-10'>
      ...
      <div class="table-cell mobile-show mobile-header">symbol_pair</div>
      <div class="table-cell mobile-cell highlight-cell">${ entry.meta_quote_currency }/${ entry.meta_base_currency }</div>

      <div class="table-cell mobile-show mobile-header">price_usd_now</div>
      <div class="table-cell mobile-cell highlight-cell dom_meta_price_usd_now"></div>

      <div class="table-cell mobile-show mobile-header">price_crypto_now</div>
      ...
      <div class="table-cell mobile-show mobile-header">total_usd_now</div>
      <div class="table-cell mobile-cell highlight-cell dom_meta_total_usd_now"></div>

      <div class="table-cell mobile-show mobile-header">price_then_share</div>
      ...
      <div class="table-cell mobile-show mobile-header">totals_then</div>
      <div class="table-cell mobile-cell">${ await domFormatEntryTotals( entry ) }</div>

      <div class="table-cell mobile-show mobile-header">price_base_then</div>
      ...
      <div class="table-cell mobile-show mobile-header">date_then</div>
      ...
      <div class="table-cell table-cell--foot mobile-cell">${ entry.input_exchange }
        <button class='delete' onclick='remove( ${ entry.id }, event )'>del</button>
      </div>
...
const getCryptoBase = async ( entry, type ) => {
  ...
  if ( entry.meta_base_currency === 'BTC' ) return '฿'
  if ( entry.meta_base_currency === 'USDT' && entry.meta_quote_currency === 'BTC' ) return '฿'
...
const domFormatCryptoPriceShare = async ( input_price_crypto_then_share, meta_base_currency ) => {
  if ( meta_base_currency === 'BTC' ) return `฿${ input_price_crypto_then_share }`
...
const clearForm = () => {
  ...
  document.querySelector( '#writeToLocalStorage' ).className = 'table-cell submit bg-g'
...
const clearLocalStorage = async () => {
  count++
  document.querySelector( '#clearLocalStorage' ).textContent = 'Are you sure? ( Click again if you are )'
  if ( count === 2 ) {
